...
ディレクトリにアクセスするためのNASオプション `.snapshot`。
 a| 
 `.snapshot`ディレクトリへのアクセスを有効または無効にします。

有効な値： `true`、 `false`。デフォルトの動作は、ONTAPおよびTridentのバージョンによって異なる場合があります。一貫した動作を保証するために、明示的に設定してください。
...
| `skipRecoveryQueue`  a| 
ボリュームの削除中は、ストレージ内のリカバリ キューをバイパスし、ボリュームを直ちに削除します。
 a| 
``
...
FlexGroupsおよびFlexGroupsに適したワークロードの詳細については、 https://www.netapp.com/pdf.html?item=/media/12385-tr4571.pdf["NetApp FlexGroupボリュームベストプラクティスおよび実装ガイド"^]を参照してください。
