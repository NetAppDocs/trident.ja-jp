...
使用する際は `ontap-nas`、 `ontap-san` 、 そして `solidfire-san`ストレージ ドライバーを使用すると、 Trident はボリュームを複製できます。使用する際は `ontap-nas-flexgroup`または `ontap-nas-economy`ドライバーの場合、クローン作成はサポートされません。既存のボリュームから新しいボリュームを作成すると、新しいスナップショットが作成されます。
