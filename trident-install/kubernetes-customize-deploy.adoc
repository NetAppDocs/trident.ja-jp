...
| 「 tridentImage 」のように入力します | インストールするTridentイメージ | `"netapp/trident:26.06"` 
...
| Prometheus メトリック エンドポイントに対して HTTPS を有効にします。 | いいえ | 「ホストネットワーク」 
|===
...
属性は<<設定オプション>>定義するとき `TridentOrchestrator`インストールをカスタマイズします。
...
WARNING: *免責事項*: この例で示されている要求値と制限値は、デモンストレーションのみを目的としています。環境とワークロードの要件に基づいてこれらの値を調整します。
