...
link:kubernetes-deploy.html["インストールの概要"]を確認して、インストールの前提条件を満たしていること、および環境に適したインストールオプションが選択されていることを確認します。

.作業を開始する前に
link:kubernetes-deploy.html#before-you-begin["導入の前提条件"]に加えてlink:https://v3.helm.sh/["Helm バージョン 3"^]が必要です。
...
----
.  `helm install`を使用して、次の例のようにデプロイメントの名前を指定します。ここで、 `100..0`はインストールするTridentのバージョンです。
+
[listing]
----
...
| `enableConcurrency`  a| 
スループット向上のために Trident コントローラの同時操作を有効にします。サポートされているドライバーの詳細については、リンク： https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html#controller-scalability[Controller Scalability documentation] を参照してください。
| いいえ 
