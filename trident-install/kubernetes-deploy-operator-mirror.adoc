...
=== 手順3：オペレータのレジストリの場所を更新します
 ...
 `/deploy/operator.yaml`で、 `image: docker.io/netapp/trident-operator:26.02.0`を更新して、イメージレジストリの場所を反映します。link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["TridentとCSIの画像"]は1つのレジストリまたは異なるレジストリに配置できますが、すべてのCSIイメージは同じレジストリに配置する必要があります。例：

* `image: <your-registry>/trident-operator:26.02.0`すべてのイメージが 1 つのレジストリにある場合。
