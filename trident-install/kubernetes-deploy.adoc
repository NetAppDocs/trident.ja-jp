...
[]
=====
* Kubernetes は Trident でサポートされるようになりました。Kubernetes をアップグレードする前に、Trident をアップグレードしてください。
...
適切なインストール方法を選択してください。決定を下す前にlink:kubernetes-deploy.html#moving-between-installation-methods["メソッド間を移動しています"]の考慮事項も確認する必要があります。
...
手動でのデプロイでもHelmを使用する場合でも、Tridentオペレーターはインストールを簡素化し、Tridentリソースを動的に管理する優れた方法です。さらに、link:kubernetes-customize-deploy.html["Tridentのオペレータ環境をカスタマイズ"]は、 `TridentOrchestrator`カスタムリソース（CR）の属性を使用して行うこともできます。
