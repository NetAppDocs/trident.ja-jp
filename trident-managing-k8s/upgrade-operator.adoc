...
[role="lead"]
Trident オペレーターを使用して、手動または Helm を使用して Trident をアップグレードできます。Trident オペレーターのインストールから別の Trident オペレーターのインストールにアップグレードすることも、 `tridentctl`のインストールから Trident オペレーターバージョンにアップグレードすることもできます。Trident オペレーターのインストールをアップグレードする前にlink:upgrade-trident.html#step-3-select-an-upgrade-method["アップグレード方法を選択します"]を確認してください。
...
.手順
. link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["Helmを使用してTridentをインストール"]の場合は、 `helm upgrade trident netapp-trident/trident-operator --version 100.2602.0`を使用してワンステップでアップグレードできます。Helmリポジトリを追加していない場合、またはアップグレードに使用できない場合：
+
.. から最新のTridentリリースをダウンロードしlink:https://github.com/NetApp/trident/releases/latest["GitHubの_Assets_sectionを参照してください"^]ます。
..  `helm upgrade`コマンドを使用します。 `trident-operator-26.02.0.tgz`にはアップグレード先のバージョンを指定します。
+
[listing]
----
...
NOTE: インストール方法を切り替える前に、link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["インストール方法を切り替える"]を確認してください。
