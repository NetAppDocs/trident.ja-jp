...
----
** PVCのみのアプリケーションを定義するには、 `PersistentVolume`と `VolumeSnapshotClass``もリソースフィルターに含める必要があります。スナップショットとバックアップ操作は、 `PersistentVolume`（各PVCにバインドされたクラスタースコープのボリューム）と `VolumeSnapshotClass`（スナップショットドライバ）に依存しており、これらがないと失敗します。例：
+
[source, console]
----
