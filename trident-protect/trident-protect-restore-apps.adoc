...
* アーカイブからすべての名前空間リソースを復元せずに特定のKubeVirt仮想マシン（VM）を復元するには、復元CR の `includedVirtualMachines`フィールドを使用します。サポートされているCR、フィールドリファレンス、およびYAMLの例については、link:trident-protect-kubevirt-vms.html["KubeVirt 仮想マシンを Trident Protect で保護する"]を参照してください。
...
** *spec.destinationApplicationName*：（_オプション_）リストアされたアプリケーションの名前。指定されている場合、リストアされたアプリケーションはこの名前を使用します。指定しない場合、リストアされたアプリケーションはソース アプリケーション名を使用します。
...
NOTE: AppVault CR がデスティネーション クラスタに存在しない場合は、link:trident-protect-appvault-custom-resources.html["Trident Protect AppVault オブジェクトを使用してバケットを管理する"]の手順に従って作成します。
...
いつでもスナップショットを元の名前空間に復元できます。In Placeリストアを実行すると、Trident Protectは、保護スケジュールと進行中の操作を自動的に管理し、無効な回復ポイントを防止します：
...
spec:
  ...
  appArchivePath: my-snapshot-path
----
+ .（オプション）アプリケーションの特定のリソースのみを選択して復元する必要がある場合は、特定のラベルが付いたリソースを含めるか除外するかを指定するフィルタリングを追加します。

+ 注記：Trident Protect は、ユーザーが選択したリソースとの関連性に基づいて、一部のリソースを自動的に選択します。例えば、永続ボリューム要求リソースを選択し、それに関連付けられたポッドがある場合、Trident Protect は、関連付けられたポッドも復元します。

+ * *resourceFilter.resourceSelectionCriteria*：（フィルタリングに必須） `Include`または `Exclude`を使用して、resourceMatchersで定義されたリソースを含めるか除外します。含めるリソースまたは除外するリソースを定義するには、以下のresourceMatchersパラメータを追加します：** *resourceFilter.resourceMatchers*：resourceMatcherオブジェクトの配列。この配列に複数の要素を定義した場合、それらはOR演算で一致し、各要素内のフィールド（group、kind、version）はAND演算で一致します。*** *resourceMatchers[].group*：（_オプション_）フィルタリングするリソースのグループ。*** *resourceMatchers[].kind*：（_オプション_）フィルタリングするリソースの種類。*** *resourceMatchers[].version*：（_オプション_）フィルタリングするリソースのバージョン。*** *resourceMatchers[].names*：（_オプション_）フィルタリングするリソースのKubernetes metadata.nameフィールドの名前。*** *resourceMatchers[].namespaces*：（_オプション_）フィルタリングするリソースのKubernetes metadata.nameフィールドにある名前空間。*** *resourceMatchers[].labelSelectors*：（_オプション_） https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors["Kubernetes のドキュメント"^]で定義されたリソースのKubernetes metadata.nameフィールド内のラベルセレクタ文字列。例： `"trident.netapp.io/os=linux"`。
