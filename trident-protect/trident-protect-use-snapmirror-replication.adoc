...
NOTE: CR を使用する場合は、CR を適用する前に、宛先名前空間を手動で作成します。Trident Protect は、CLI を使用する場合にのみ名前空間を自動的に作成します。
...
*** *spec.storageClassMapping*: （_オプション_） 同じ AMR 内の個々の PVC のソースストレージクラスをデスティネーションストレージクラスにマッピングします。各エントリには `source`および `destination`ストレージクラス名が必要です。設定すると、 `storageClassMapping`は `storageClassName`を完全にオーバーライドします。一致するソースエントリのない PVC はクラスタのデフォルトのストレージクラスを使用し、 `storageClassName`は使用しません。
+
AppMirrorRelationship CR で `storageClassMapping`を使用する際は、以下の制限事項に注意してください。

+
**** `storageClassMapping`は `storageClassName`を完全にオーバーライドします。両方のフィールドが設定されている場合、 `storageClassName`は無視されます。
****  `storageClassMapping`に一致するエントリがないソースストレージクラスを持つ PVC は、 `storageClassName`ではなく、クラスターのデフォルトストレージクラスを使用します。
**** 各エントリーには、 `source`と `destination`の値が必要です。
**** デスティネーション ストレージクラスを変更すると、既存のデスティネーション PVC の削除と再作成が強制される場合があります。
...
NOTE:  `--storageclass-mapping`を使用して、個々のソースストレージクラスをデスティネーションストレージクラスにマッピングします。カンマで区切られた1つ以上の `source:destination`ペアを指定します（例： `"class-a:class-b,class-c:class-d"`）。 `*:destination-class`をワイルドカードとして使用して、マッピングされていないすべてのPVCを単一のデスティネーションストレージクラスにマッピングします。 `--storageclass-mapping`が設定されている場合、 `--storage-class`は無視されます。

+
*単一のデスティネーション ストレージクラスを使用した例(`--storage-class`）：*
