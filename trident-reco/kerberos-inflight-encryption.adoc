...
* *オンプレミス ONTAP* - Trident は、NFSv3 における Kerberos 暗号化と、Red Hat OpenShift および upstream Kubernetes クラスタからオンプレミス ONTAP ボリュームへの接続をサポートします。
...
* Kerberos暗号化を使用するボリュームはすべて正しく設定されていることを確認してください。NetAppドメイン構成セクション（13ページ）を参照してください https://www.netapp.com/media/16398-tr-3580.pdf["NetApp 機能強化とベストプラクティスガイド"^]。
...
.アクセスプロトコル
エクスポートポリシーをNFS、NFSv3、およびアクセスプロトコルで構成します。
