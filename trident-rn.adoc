...
NetApp TridentおよびTrident Protectの新機能（機能強化、修正、廃止を含む）について説明します。
...
* * Kubernetes *：
+
** CSIの公開とボリュームの移動の間の競合状態を修正しました。この問題により、古いアクセス情報を使用してボリュームがアタッチされる可能性がありました。
** NodeGetInfo がノード登録完了前に空のトポロジーラベルを返す可能性があった競合状態を修正し、トポロジーを考慮したプロビジョニングの失敗を防ぎます。
** 別のポッドがまだ接続中の間に共有サブシステムを切断してしまう可能性があった、まれな NVMe 切断競合を修正しました。
** 同時実行環境でマウントされたボリュームのクローンを作成する際に発生する、NVMe PVCクローンの公開失敗（および親ネームスペースのマッピングの誤り）を修正しました。
** nameTemplateを使用したONTAP-SAN-EconomyバックエンドでのPVCのスナップショット作成エラーを修正しました。
...
* * Kubernetes *：
+
** オプション `storageAPITimeout`（秒）をONTAPバックエンド設定に追加し、バックエンドごとにRESTおよびZAPI APIクライアントのタイムアウトを設定できるようにしました。
...
* * Kubernetes *：
+
** ONTAP-NAS-EconomyおよびONTAP-SAN-Economyドライバーの同時実行に対する一般提供（GA）サポートを追加し、この機能をテクニカルプレビューから正式版に移行しました。また、Azure NetApp Files（ANF）およびSolidFireドライバーの同時実行に対するGAサポートも追加しました。
** Kubernetes 1.36 のサポートが追加されました。
** Google Cloud NetApp Volumes （GCNV）NASおよびSANドライバでのボリュームの自動拡張に対する一般提供（GA）サポートを追加しました。
** 新しいボリュームのプロビジョニング時に最適なアグリゲートを選択するための統合 ONTAP バランス配置を追加しました（link:https://github.com/NetApp/trident/issues/64["問題 #64"]）。
** ONTAP-NAS-Flexgroup ドライバに ONTAP AFX サポートを追加しました。
** Red Hat OpenShift Migration Toolkit for Virtualizationを使用する際の仮想マシンの移行を簡素化するため、NetApp Shift Toolkitとの統合を追加しました。
** GCNV SANドライバにクラウドIDのサポートを追加しました。
** Trident Volume Publication （TVP）の伝播およびボリュームの自動拡張ワークフローに対して、従属ボリュームのサポートを追加しました。
** 同時実行が有効な状態で Trident を実行する際のボリューム更新およびボリューム グループ スナップショットのサポートを追加しました。
** GCNV向けに動的なgRPCレート制限機能を追加しました。
** コントローラーの準備状況実行プローブを追加し、コントローラーとサイドカーのデフォルトのKubernetes API QPSおよびバースト設定を調整しました。
** アタッチ、暗号化設定、ファイルシステムフォーマットの各ステップを分離することで、ノード操作のシーケンスを改善しました。
** セキュリティ向上のため、マウントされた Trident コンテナのルートファイルシステムを読み取り専用にしました。
** ONTAP バックエンドでクローンボリュームのソーススナップショットが削除された際に、クローンボリュームスナップショットのベストエフォート型スナップショット削除機能を追加しました。
** Veeam Kasten、Trilio、および Cohesity によって作成された一時的な PVC が ONTAP ボリューム リカバリ キューでスタックしないように、バックアップ クローン PVC の認識を追加しました。
** 特殊リージョンをサポートするために、AWS ARNの処理を強化しました。
** ONTAP-SAN-ECONOMYドライバのサイズ変更操作中にFlexvolが縮小するのを防ぎます。
...
* *[Tech Preview]*: ONTAP-NAS-Economy qtreeインポートのサポートを追加しました。
* *[Tech Preview]*: ASA-R2 (SAN および NVMe) ドライバーの同時実行のサポートを追加しました。
* *[Tech Preview]*: ONTAP-SANおよびONTAP-NASドライバ向けに、GCNV ONTAPモードのHTTP転送を追加しました。
...
* * Kubernetes *：
+
** 同時実行される公開/非公開、クローン、キャッシュ、およびバックエンド更新操作における競合状態と状態処理を修正しました。
** ONTAP-NASおよびONTAP-NAS-Economyドライバにおける、サブボリュームおよび読み取り専用クローンに対するエクスポートポリシーの競合状態と同時パブリッシュ/アンパブリッシュ処理の問題を修正しました。
** スナップショット ポリシーが異なる場合の、MetroClusterフェイルオーバー後のONTAP-NAS-Economy FlexVolの検出を修正しました（link:https://github.com/NetApp/trident/issues/1082["問題 #1082"]）。
** ONTAP-NAS-Economy ボリューム削除の再試行により、長時間実行中の qtree 削除が中断される問題を修正しました（link:https://github.com/NetApp/trident/issues/1121["問題 #1121"]）。
** ONTAP-SANおよびONTAP-SAN-Economyのインポートおよびサイズ変更の動作を修正しました。これには、fsTypeの検証、ボリュームメタデータの処理、および自動拡張モードの動作が含まれます。
** 同時ボリューム作成時のNVMe/TCPネームスペース競合状態を修正しました（link:https://github.com/NetApp/trident/issues/1089["問題 #1089"]）。
** 利用可能なパスとポータルが一致しない場合に発生していたiSCSIサイズ変更の失敗を修正し、繰り返しサイズ変更時の動作を改善しました。
** ボリューム使用状況が原因で発生する、ONTAP-SANボリュームの削除失敗を修正しました。
** ONTAP REST エラー処理を修正し、SVM 状態タイムアウトの動作を改善しました。
** GCNV SAN のゾーンプールとホストグループの処理を修正し、ボリューム識別子の変換を正規化し、最小サイズ未満の作成リクエストを正規化しました。
** ネイティブGCNV容量プールの選択において、ONTAPモードのプールを除外するように修正しました。
** KubeVirt の VM フェイルオーバーを修正しました。Trident 自動ワークロードフェイルオーバーを使用して VM フェイルオーバーを高速化します。
** オペレーター管理インストールのRBACに関する問題を修正しました。これには、ノードのClusterRolesの欠落、およびアップグレード時の不正なClusterRoleとClusterRoleBindingの調整が含まれます。これは、Trident Operatorチャートを使用するOperatorおよびHelmベースのインストールに適用されます。
** デフォルトの優先度値を省略し、YAMLテンプレートのインデントを修正することで、TridentConfiguratorおよび関連するCRDにおける永続的なGitOpsの差分を修正しました（link:https://github.com/NetApp/trident/issues/915["問題 #915"]、link:https://github.com/NetApp/trident/issues/1120["問題 #1120"]）。
** Helm アップグレード中の TridentVolumePublication CRD のパッチ適用を修正しました（link:https://github.com/NetApp/trident/issues/1151["問題 #1151"]）。
** ノード登録の再試行前にCSI gRPCサービスを開始することで、CSIの起動順序を修正しました。
** 同時実行時のセキュアSMBクローン構成オーバーレイを修正しました。
** アップグレード後に Trident コントローラーポッドに注釈が欠落していた問題を修正しました（link:https://github.com/NetApp/trident/issues/1004["問題 #1004"]）。
** 大規模SVM上での低速なqtreeプロビジョニングを改善しました（link:https://github.com/NetApp/trident/issues/1142["問題 #1142"]）。
** ControllerPublish が誤ったデフォルトのファイルシステムタイプではなく、ボリューム構成のファイルシステムタイプを使用するように修正しました。
** 孤立した dm デバイスが CSI NodeUnstageVolume の繰り返し失敗を引き起こす可能性がある問題を修正しました。
** ゴーストマルチパスアーティファクトおよび古い TridentNode リソースをソフト削除されたノードのクリーンアップ後に削除することで、古いノードとマルチパスのクリーンアップを改善しました。
...
* OpenShift Service CA Operator との統合を追加し、Trident Protect アドミッション Webhook の TLS 証明書管理を自動化します。Helm の値で `useOpenshiftServiceCA`を `true`に設定することで有効にします。詳細については、link:trident-protect/trident-protect-customize-installation.html["Trident Protectのインストールをカスタマイズする"^]を参照してください。
* OpenShift VirtualizationでのKubeVirt VMのバックアップとリストアのサポートが追加されました。ネームスペース内のすべてのVM、または名前で指定した特定のVMを保護し、バックアップまたはスナップショットから個々のVMを選択的にリストアできます。詳細については、link:trident-protect/trident-protect-kubevirt-vms.html["KubeVirt 仮想マシンを Trident Protect で保護する"^]を参照してください。
* リソース変換のサポートを追加し、復元プロセス中にKubernetesリソース属性を追加、削除、変更できるようにしました。詳細については、link:trident-protect/trident-protect-resource-transformations.html["カスタムリソースを使用して復元する際にリソースを変更します"]を参照してください。
* AppMirrorRelationship（AMR）での `storageClassMapping`のサポートが追加され、単一の AMR 定義内で個々の PVC のストレージクラスマッピングが可能になりました。詳細については、link:trident-protect/trident-protect-use-snapmirror-replication.html["NetApp SnapMirrorとTrident Protectを使用してアプリケーションを複製する"^]を参照してください。
...
* ソースアプリケーションの名前空間がデスティネーション クラスタに存在しない場合に、クラスター間仮想マシンのリストアがアドミッション Webhook エラーで失敗する問題を修正しました。
* 既存のアプリケーションカスタムリソースとの仕様の不一致が原因で、アプリケーション作成ステップで選択的な仮想マシンの In Place リストアが失敗する問題を修正しました。既存のアプリケーションが再利用されるようになりました。
* 仮想マシンとその PersistentVolumeClaim の両方が削除された場合に、仮想マシンの In Placeリストア ジョブが `Running`状態でハングする問題を修正しました。PersistentVolumeClaim が存在しない場合は、致命的なエラーとして処理されるようになりました。
* 既存の保護スケジュールを編集した際に、誤って即座にバックアップまたはスナップショットが実行されてしまう問題を修正しました。
* スナップショットファイナライザーが、アプリケーションが削除された後にそのカスタムリソースを取得しようとする問題を修正しました。アプリケーションが見つからない場合、ファイナライザーは処理をスキップするようになりました。
* PersistentVolumeClaimのコピー手順でスパースコピーを使用した仮想マシンのIn Placeリストア後に、保護された固定ストレージ容量が大幅に増加する問題を修正しました。
* Trident Protect が ONTAP のバッキング スナップショットの削除を確認する前に Kubernetes スナップショット オブジェクトを削除していた問題を修正しました。ファイナライザーは `VolumeSnapshot`削除を待機するようになりました。
* 仮想マシンのフリーズ操作が失敗した場合にスナップショット処理が進まない問題を修正しました。フリーズおよびフリーズ解除操作は、失敗した場合でも続行されるようになりました。
* AppMirrorRelationshipの反転、反転再同期、および再同期の操作中に、KubeVirt仮想マシン アプリケーションがPersistentVolumeClaimsを誤って再作成する問題を修正しました。
...
* * Kubernetes *：
* Google Cloud NetApp Volumes（GCNV）SANの動作を修正し、ブロック（iSCSI）ワークロード用のFlex Unified ZONALプールをサポートするようにしました。ゾーン構成とリージョン構成の両方がサポートされるようになりました。
* Google Cloud NetApp Volumes（GCNV）NAS UNIFIEDプールの処理を修正しました。 `InternalID`が存在しない場合のボリュームの作成、検証、命名、およびフォールバックボリューム検索操作を改善しました。
* 利用可能なパスとポータルが一致しない場合に、iSCSIデバイスのサイズ変更操作が失敗する可能性がある問題を修正しました。
*  `blkid`がLUKSデバイスのファイルシステムタイプを誤って識別し、新しいボリュームの作成ができなかった断続的な問題を修正しました。
* 大規模な LUKS のパブリッシュおよびステージング操作が Trident コントローラをブロックする可能性がある問題を修正しました。ユーザーは、ボリュームに関連付けられた LUKS パスフレーズを追跡することが求められるようになりました。
* iSCSIマルチパスパーティションがゴーストデバイスとして表示され、CSI `NodeUnstage`処理が成功しない問題を修正しました。
* 同時ボリューム作成時のNVMe/TCPネームスペース競合状態を修正しました（問題#1089）。
* 同じ秒内にスナップショットが実行された場合の同時クローンスナップショット操作を修正しました。
* KubeVirt VM が `dataVolumes`を使用して Trident Automated Workload Failover でフェイルオーバーできない問題を修正しました。
* Trident並行コアの使用中に無効な `TridentMirrorRelationship`カスタムリソースを削除する際に発生したエラーを修正しました。
*  `tridentactionmirrorupdates` Kubernetes CRD YAML定義のインデントを修正しました（問題#1120）。
* REST qtree非同期削除API呼び出しを修正しました（問題#1121）。
*  `ControllerPublish`誤ったデフォルトのファイルシステムタイプではなく、ボリューム構成のファイルシステムタイプを使用するように修正しました。
* コントローラの起動ログを修正し、同時ドライバのステータスを正しく報告するようにしました。
* 並行処理における中断されたバックエンドの処理を修正しましたTrident。
* 特殊リージョンをサポートするために、AWS ARNの処理を強化しました。
...
セキュリティ上の脆弱性を解決するために、Trident Protectの依存関係を更新しました：

* gRPC-Go：link:https://nvd.nist.gov/vuln/detail/CVE-2026-33186["CVE-2026-33186を使用したチャンク アップロード署名要求がサポートされるようになりました。"^]
* Go x/crypto SSH：link:https://nvd.nist.gov/vuln/detail/CVE-2024-45337["CVE-2024-45337を使用したチャンク アップロード署名要求がサポートされるようになりました。"^]
* OpenTelemetry-Go：link:https://nvd.nist.gov/vuln/detail/CVE-2026-24051["CVE-2026-24051"^]
* Go ランタイム：link:https://nvd.nist.gov/vuln/detail/CVE-2025-22871["CVE-2025-22871を使用したチャンク アップロード署名要求がサポートされるようになりました。"^]
* 追加の依存関係：link:https://nvd.nist.gov/vuln/detail/CVE-2025-23109["CVE-2025-23109を使用したチャンク アップロード署名要求がサポートされるようになりました。"^]
...
* * Kubernetes *：
+
** ONTAP-NAS（NFSのみ）、ONTAP-SAN（iSCSI、FCP、NVMe）、およびGoogle Cloud NetApp Volumes（GCNV）ドライバの同時実行に対する一般提供（GA）サポートを追加し、この機能をTech Previewから正式版に移行しました。詳細については、link:https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html["コントローラのスケーラビリティに関するドキュメント"]を参照してください。
** カスタムのユーザー定義Trident AutoGrowポリシーに基づくTridentでのボリュームの自動拡張のサポートを追加しました。詳細については、link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html["自動ボリューム拡張ドキュメント"]を参照してください。
** 強化されたTridentノードの同時実行性により、NVMe ボリュームのノード操作のスケーラビリティが向上します。詳細については、link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html["NVMeボリュームのドキュメント"]を参照してください。
** PVCアノテーション(`tieringPolicy` `tieringMinimumCoolingDays`を使用したGoogle Cloud NetApp Volumes NASボリュームの自動階層化のサポートが追加されました。プールの選択とクローンの継承が含まれます。詳細については、link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-auto-tiering.html["Google Cloud NetApp Volumes ドキュメントの自動階層化を設定する"]を参照してください。
**  `google-cloud-netapp-volumes-san`ドライバを使用したGCNV SANブロック（iSCSI）ボリュームのサポートが追加されました。プロビジョニング、LUNアクセス用のノード単位のホストグループマッピング、ボリュームからのクローン操作が含まれます。詳細については、link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-block.html["Google Cloud NetApp Volumes ブロック構成ドキュメント"]を参照してください。
** Amazon FSx for NetApp ONTAP の自動バックエンド構成のサポートを追加しました。必要なパラメータを含む StorageClass を作成すると、Trident は対応するバックエンドと、必要に応じて VolumeSnapshotClass を自動的に作成します。詳細については、link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems["Amazon FSx for NetApp ONTAPバックエンド構成ドキュメント"] を参照してください。
** Azure GovernmentやAzure ChinaなどのさまざまなMicrosoft Azureクラウドのサポートと、Azure NetApp Filesバックエンドのカスタムクラウド構成を追加しました。詳細については、link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Azure NetApp Files バックエンド構成ドキュメント"]を参照してください。
** Kubernetes 1.35 のサポートが追加されました。詳細については、link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html["要件ドキュメント"]を参照してください。
...
* *[Tech Preview]*：ONTAP-NAS-Economy および ONTAP-SAN-Economy ドライバーの同時実行のサポートが追加されました。
...
[WARNING]
====
 `csi-snapshotter`サイドカーには既知の問題があります。すべてのKubernetesバージョンにおいて、VolumeGroupSnapshots `v1beta1`がVolumeSnapshotsの `ReadyToUse`状態への到達を防ぎます。

====
回避策は 2 つあります：

. VolumeGroupSnapshots CRDを削除してVolumeGroupSnapshotsを無効にし、Tridentを再インストールします。
. VolumeGroupSnapshots `v1beta2`および `snapshot-controller`バージョン8.4.0以降をインストールしてから、Tridentを再インストールしてください。VolumeGroupSnapshotsはKubernetesバージョンv1.34より前では機能しません。
...
* * Kubernetes *：
+
** ONTAP-NAS、ONTAP-NAS-Economy、Google Cloud NetApp Volumesドライバで読み取り専用クローンを非公開にすると、ソース ボリュームからエクスポート ポリシー ルールが削除される問題を修正しましたlink:https://github.com/NetApp/trident/issues/1086["問題 #1086"]。
** Bitnami のパブリック イメージの廃止に伴うプルの失敗を防ぐため、kubectl イメージを軽量の Alpine ベースのバリアントに切り替えました https://github.com/NetApp/trident/issues/1080["問題 #1080"]。
** Tridentアップグレード中の既存のデプロイメントの注釈の保持を修正link:https://github.com/NetApp/trident/issues/1004["問題 #1004"]。
** 両方のストレージ クラスが同じバックエンドを指している場合、異なるストレージ クラス間でのクローン作成を許可するlink:https://github.com/NetApp/trident/issues/1104["問題 #1104"]。
** ネットワーク遅延のあるクラウド環境でのタイムアウトによって発生するノード準備の失敗を修正しました。クラウドベースのインストールのタイムアウト値を増やしました。
** プロセスが再試行状態になったときにファイルシステム タイプ属性が設定されないままになる LUN 作成の問題を修正しました。
** REST APIボリューム ルックアップを修正し、ボリューム状態を無視することで、ボリューム クエリ中の誤検出を防止しました。
** 大規模に使用する場合の ontap-nas-economy ドライバーの Trident コントローラー効率が向上しました。
** ontap-san-economy ドライバで LUN インポート時に internalID を設定します。
** より多くのサブネットを処理できるように、Azure Resource Graph のクエリ制限が引き上げられました。
** 一部のバックアップアプリケーションとの競合状態を回避するために、CSIとONTAP クローン スプリットのタイムアウトを改善しましたlink:https://github.com/NetApp/trident/issues/1098["問題 #1098"]、link:https://github.com/NetApp/trident/issues/1100["問題 #1100"]。
** LUKSエラーメッセージの抑制を修正link:https://github.com/NetApp/trident/issues/1069["問題 #1069"]。
** iSCSI プロトコルと NVMe プロトコルの両方における古い LUKS マッパーの処理を修正しました。強化されたクリーンアップ ロジックにより、孤立したデバイス マッパーによるマウントの失敗を防止します。


* RWX NVMe ボリュームのスケール制限を修正しました。
* OpenTelemetry-Goパッケージを更新してlink:https://nvd.nist.gov/vuln/detail/cve-2026-24051["CVE-2026-24051"]を修正しました。
...
* Trident Protectは、In Placeリストアの前に保護スケジュールを自動的に無効にし、進行中の処理をキャンセルして、リストアの完了後に再度有効にします。詳細については、link:trident-protect/trident-protect-restore-apps.html["Trident Protectを使用してアプリケーションを復元する"^]を参照してください。
*  `runImmediately`フィールドをスケジュールCRに追加し、 `--run-immediately` CLIフラグを追加して、スケジュールの作成時に即時バックアップまたはスナップショットをトリガーできるようにしました。詳細については、link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule["データ保護スケジュールを作成"^]を参照してください。
* リストア CR の `destinationApplicationName`フィールドまたは `--destination-app-name` CLI フラグを使用して、リストアされたアプリケーションのカスタム名を指定するためのサポートが追加されました。詳細については、link:trident-protect/trident-protect-restore-apps.html["Trident Protectを使用してアプリケーションを復元する"^]を参照してください。
...
* 必要なサービス アカウントが使用可能になる前にポッドが作成されたために発生するリストアの失敗を修正しました。
*  `Roles`と `RoleBindings`がアプリケーションのリストア中にスキップされる問題を修正しました。
* 正しく設定されているにもかかわらず、 `tridentctl-protect get appvaultcontent`出力に元のクラスタ名が表示されない問題を修正しました。
* 欠落した `pipefail`処理により無視されていたKopiaリストアエラーを修正しました。
* 永続ボリュームを除外するリソース フィルタによって発生するスナップショットおよびバックアップの障害を修正しました。
* ネームスペース全体で同じ名前のPVCを持つマルチネームスペースアプリケーションにおいて、データ損失につながる可能性があった誤ったPVCリストアを修正しました。
...
+
見るlink:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["サポートされているバックアップとスケジュールの注釈"^]。



* バックアップ CR によって使用される PVC バインド タイムアウトを構成するための注釈をスケジュール CR に追加しました。 `protect.trident.netapp.io/pvc-bind-timeout-sec` 。見るlink:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["サポートされているバックアップとスケジュールの注釈"^]。
...
IMPORTANT: SolidFireをご利用のお客様は、ボリュームの非公開時における既知の問題があるため、25.06.1にアップグレードしないでください。この問題に対処するため、25.06.2がまもなくリリースされる予定です。
...
** GCNV ボリュームが `nfsMountOptions`未設定時に NFSv3 にデフォルト設定される問題を修正しました。現在、NFSv3 と NFSv4 の両方のプロトコルがサポートされています。 `nfsMountOptions`が指定されていない場合、ホストのデフォルトの NFS バージョン（NFSv3 または NFSv4）が使用されます。
...
* volumeMode: File および volumeMode: Block (raw デバイス) ストレージの両方に対して、KubeVirt / OpenShift Virtualization VM のバックアップと復元のサポートが追加されました。このサポートはすべてのTridentドライバーと互換性があり、 Trident Protect を備えたNetApp SnapMirrorを使用してストレージを複製する際の既存の保護機能を強化します。
