...
* ONTAP-NAS（NFSのみ）、ONTAP-SAN（iSCSI、FCP、NVMe）、およびGoogle Cloud NetApp Volumesドライバの同時実行サポートが一般提供（GA）されました。
* Amazon FSx for NetApp ONTAP の自動バックエンド構成のサポートを追加しました。必要なパラメータを含む StorageClass を作成すると、NetApp Trident が対応するバックエンドと VolumeSnapshotClass（必要な場合）を自動的に作成します。
* Azure GovernmentやAzure ChinaなどのさまざまなMicrosoft Azureクラウドのサポートと、Azure NetApp Filesバックエンドのカスタムクラウド構成を追加しました。
* NVMe ボリュームのコントローラのスケーラビリティが向上しました。
* ボリュームの自動拡張機能のサポートを追加しました。カスタムNetApp Trident自動拡張ポリシーを定義することで、自動拡張の動作を制御できます。
* Google Cloud NetApp Volumes ブロック ストレージ（SAN/iSCSI）のサポートを追加しました。
* Google Cloud NetApp Volumes の自動階層化のサポートを追加しました。Kubernetes ネイティブコントロールを追加して ONTAP FabricPool の自動階層化を設定することで、NetApp Trident を通じて Google Cloud NetApp Volumes のストレージコストを最適化できるようになりました。
* Kubernetes 1.35 のサポートが追加されました。
