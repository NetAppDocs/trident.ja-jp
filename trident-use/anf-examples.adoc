...
| `maxThroughput` | 許容される最大スループットを設定します。手動QoS容量プールでのみサポートされます。* Trident 25.06のテクニカルプレビュー* | `4 MiB/sec` 
|===
...
[%collapsible]
====
このバックエンド構成では、Azureの `eastus`手動 QoS 容量プールを備えた場所。* NetApp Trident 25.06 のテクニカル プレビュー*。
