...
| 「スナップショット方向」 | 「 .snapshot 」ディレクトリにアクセスします | `true`、 `false` （デフォルトの動作は異なる場合があります。明示的に設定）NFSv3の場合は「false」 
...
NOTE:  `<id_value>`と `<key_value>`をサービスアカウントの認証情報に置き換えます。
...
[%collapsible]
====
GKEクラウドIDを使用してTridentをインストールすると、バックエンドはワークロードIDとして認証され、 `apiKey`ブロックは省略されます。クラウドIDは、両方のGoogle Cloud NetApp Volumesドライバーで同じように設定されます。唯一の違いは `storageDriverName`の値です。NAS（NFSまたはSMB）ボリュームには `google-cloud-netapp-volumes`を使用し、SAN（iSCSIブロック）ボリュームには `google-cloud-netapp-volumes-san`を使用します。


NOTE:  `google-cloud-netapp-volumes-san`ドライバーのクラウドID サポートには、Trident 26.06以降が必要です。SANボリュームはFlex Unifiedストレージプールからプロビジョニングされるため、 `serviceLevel`を `flex`に設定してください。

.NASドライバー（<code>google-cloud-netapp-volumes</code>）
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
...
----
.SANドライバ（<code>google-cloud-netapp-volumes-san</code>）
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
