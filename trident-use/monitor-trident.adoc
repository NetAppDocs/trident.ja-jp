...
NOTE: デフォルトでは、Tridentのメトリクスはターゲットポートに公開されます。 `8001`で `/metrics`終点。これらのメトリックは、 Tridentがインストールされると *デフォルトで有効* になります。ポートでHTTPS経由でTridentメトリックを使用するように設定できます `8444`同じように。見るlink:..https://docs.netapp.com/us-en/trident/trident-use/monitor-trident.html#step-2-create-a-prometheus-servicemonitor["手順 2 ： Prometheus ServiceMonitor を作成します"^]詳細についてはこちらをご覧ください。
