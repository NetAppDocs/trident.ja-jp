...
NOTE: デフォルトでは、Tridentのメトリクスはターゲットポートに公開されます。 `8001`で `/metrics`終点。これらのメトリックは、 Tridentがインストールされると *デフォルトで有効* になります。ポートでHTTPS経由でTridentメトリックを使用するように設定できます `8444`同じように。
...
メトリックを収集し、 Trident が管理するバックエンドや作成するボリュームなどの情報を取得するには、Prometheus ターゲットを定義する必要があります。見るlink:https://prometheus-operator.dev/docs/developer/getting-started/["Prometheus Operator ドキュメント"^]。
...
=== HTTPS経由でTridentメトリクスを消費する

HTTPS (ポート 8444) 経由でTridentメトリックを使用するには、ServiceMonitor 定義を変更して TLS 構成を含める必要があります。また、 `trident-csi`秘密から `trident`Prometheus が実行されている名前空間への名前空間。次のコマンドを使用してこれを実行できます。

`kubectl get secret trident-csi -n trident -o yaml | sed 's/namespace: trident/namespace: monitoring/' | kubectl apply -f -`

HTTPS メトリックのサンプル ServiceMonitor は次のようになります。
...
----
Trident は、tridentctl、Helm chart、Operator など、すべてのインストール方法で HTTPS メトリックをサポートします。

* を使用している場合 `tridentctl install`コマンドを実行すると、 `--https-metrics` HTTPS メトリックを有効にするフラグ。
* Helmチャートを使用している場合は、 `httpsMetrics` HTTPS メトリックを有効にするパラメータ。
* YAMLファイルを使用している場合は、 `--https_metrics`旗に `trident-main`コンテナ内の `trident-deployment.yaml`ファイル。
...
* Trident AutoSupportは、個人を特定できる情報（PII）または個人情報を収集または送信しません。付属の https://www.netapp.com/pdf.html?item=/media/14114-enduserlicenseagreementworldwide.pdf["EULA"^]は、Tridentコンテナイメージ自体には適用されません。NetAppのデータセキュリティと信頼への取り組みの詳細については、 https://www.netapp.com/pdf.html?item=/media/14114-enduserlicenseagreementworldwide.pdf["こちらをご覧ください"^]を参照してください。
