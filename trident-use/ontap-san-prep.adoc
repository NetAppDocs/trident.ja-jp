...
NOTE: link:https://docs.netapp.com/us-en/asa-r2/get-started/learn-about.html["ASA r2システム"^]ストレージ層の実装は他のONTAPシステム（ASA、AFF、FAS）とは異なります。ASAは、アグリゲートの代わりにストレージアベイラビリティゾーンが使用されます。link:https://kb.netapp.com/on-prem/ASAr2/ASAr2_KBs/su_create_by_SVM_admin_using_CLI_fails_with_error_No_candidate_aggregates_are_available_for_storage_services["これ"^] ASA r2 システムで SVM にアグリゲートを割り当てる方法に関するナレッジベースの記事。
