...
[role="lead"]
Trident は、AWS FSx for NetApp ONTAP（FSxN）の自動バックエンド設定をサポートしています。必要なFSxNパラメータを含むStorageClassを作成すると、Tridentは対応するバックエンドとVolumeSnapshotClassを自動的に作成します。
...
バックエンドパラメータの完全なリストについては、以下を参照してください：xref:trident-fsx-storage-backend.html#backend-advanced-configuration-and-examples[FSx for NetApp ONTAP バックエンド構成]
