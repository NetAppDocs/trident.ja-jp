...
Tridentを使用してボリュームをプロビジョニングする前に、Tridentバックエンド構成（TBC）を作成して、ストレージバックエンドをTridentと統合する必要があります。TBCは、Tridentがストレージバックエンドに接続する方法を定義し、Tridentがストレージリソースを管理するために必要な情報を提供します。



[[create-a-trident-backend-configuration-tbc-for-fsx-for-ontap]]
== FSx for ONTAP 用の Trident バックエンド構成（TBC）を作成します

Trident バックエンドを設定して FSx for ONTAP ファイルシステムを使用する方法は 2 つあります：* 自動バックエンド機能を活用する StorageClass を作成します。* バックエンドと StorageClass を個別に作成します。

このセクションの手順では、バックエンドを個別に作成する方法について説明します。自動バックエンド機能の使用方法については、link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration["StorageClassとPVCを設定する"]セクションを参照してください。
...
| 'torageDriverName' | ストレージドライバの名前。作成後は変更できません。このパラメータを更新するには、新しいバックエンドを作成する必要があります。 | `ontap-nas`、 `ontap-nas-economy`、 `ontap-nas-flexgroup`、 `ontap-san`、 `ontap-san-economy` 
...
| 「重複排除 | プロトコル LIF の IP アドレス。*ONTAP NAS ドライバー*：NetApp は dataLIF を指定することを推奨します。指定しない場合、Trident は SVM から dataLIF を取得します。NFS マウント操作に使用する完全修飾ドメイン名（FQDN）を指定することで、ラウンドロビン DNS を作成し、複数の dataLIF 間で負荷分散を行うことができます。初期設定後でも変更可能です。参照 https://kb.netapp.com/Cloud/Astra/Trident/Is_it_possible_to_update_the_DataLIF_on_the_trident_backend["初期設定後にデータLIFを更新する"^]。*ONTAP SAN ドライバー*：iSCSI には指定しないでください。Trident は ONTAP Selective LUN Map を使用して、マルチパスセッションの確立に必要な iSCSI LIF を検出します。dataLIF が明示的に定義されている場合、警告が生成されます。Trident が IPv6 フラグを使用してインストールされている場合、IPv6 アドレスを使用するように設定できます。IPv6 アドレスは角括弧で囲んで定義する必要があります（例：[28e8:d9fb:a825:b7bf:69a8:d02f:9e7b:3555]）。 |  
...
| 'toragePrefix' | SVM で新しいボリュームをプロビジョニングする際に使用するプレフィックスを指定します。作成後に変更することはできません。このパラメータを更新するには、新しいバックエンドを作成する必要があります。 | `trident`注意：バックエンドIDとボリューム命名に影響を与えるバックエンド構成パラメーター（ `storageDriverName`や `storagePrefix`など）は、作成後に変更することはできません。これらを更新するには、新しいバックエンドを作成する必要があります。 
...
| `luksEncryption` | LUKS暗号化を有効にします。link:../trident-reco/security-luks.html["Linux Unified Key Setup（LUKS；統合キーセットアップ）を使用"]を参照してください。SANのみ。 | "" 
...
SMB ボリュームは、 `ontap-nas`ドライバを使用してプロビジョニングできます。link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details["ONTAP SAN および NAS ドライバの統合"]を完了する前に、次の手順を実行します（link:worker-node-prep.html#prepare-to-provision-smb-volumes["SMBボリュームをプロビジョニングする準備をします"]）。
