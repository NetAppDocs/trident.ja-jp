...
このセクションの手順では、バックエンドを個別に作成する方法について説明します。自動バックエンド機能の使用方法については、xref:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[StorageClassとPVCを設定する]セクションを参照してください。
...
SMBボリュームをプロビジョニングするには、 `ontap-nas`ドライバ。完了する前に <<ONTAP SAN and NAS driver integration>> 次の手順を実行します。 link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["SMBボリュームをプロビジョニングする準備をします"]。
