...
このセクションの手順では、バックエンドを個別に作成する方法について説明します。自動バックエンド機能の使用方法については、link:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap["StorageClassとPVCを設定する"]セクションを参照してください。
...
SMB ボリュームは、 `ontap-nas`ドライバを使用してプロビジョニングできます。link:trident-fsx-storage-backend.html#ontap-san-and-nas-driver-integration["ONTAP SAN および NAS ドライバの統合"]を完了する前に、次の手順を実行します（link:worker-node-prep.html#prepare-to-provision-smb-volumes.html["SMBボリュームをプロビジョニングする準備をします"]）。
