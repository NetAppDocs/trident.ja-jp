...
** ストレージクラスの再利用ポリシーが `delete`の場合、PVが削除されると、Tridentはストレージボリュームを削除します。


* デフォルトでは、Trident は PVC を管理し、バックエンドの FlexVol ボリュームおよび LUN または qtree の名前を変更します。 `--no-manage` フラグを渡すと管理対象外のボリュームをインポートでき、 `--no-rename` フラグを渡すとボリューム名を保持できます。
+
...
**  `--no-rename`* -  `--no-rename`フラグを使用する場合、Trident はボリュームをインポートする際に既存のボリューム名を保持し、ボリュームのライフサイクルを管理します。このオプションは、 `ontap-nas`、 `ontap-san`（ASA r2 システムを含む）、 `ontap-san-economy`、および `ontap-nas-economy`（qtree インポート）ドライバーでのみサポートされています。
+
...
[NOTE]
====
*  `ontap-nas-economy`ドライバーはqtreeレベルでストレージを管理するため、単独でのFlexVol volumeのインポートはサポートしていません。 `ontap-nas-economy`ドライバーを使用してqtreeをインポートするには、<<import-qtrees,qtree をインポート>>（Tech Preview）を参照してください。Tridentがqtreeをインポートする際、そのqtreeを含む親FlexVolもバケットとしてインポートします。
...
[NOTE]
====
qtree インポートは Trident 26.06 のテクニカルプレビュー機能です。テクニカルプレビュー機能は、本番環境での使用はサポートされていません。

====
Trident 26.06以降、 `ontap-nas-economy`ドライバを使用して、既存のONTAP qtreeをTidentにインポートできます。FlexVol volumeとqtree名を `flexvol/qtree`形式で指定し、それを含むバックエンドのUUIDとともにqtreeを識別します。<<概要と考慮事項>>の一般的なインポートに関する考慮事項は、qtreeのインポートにも適用されます。

.tridentctlを使用してqtreeをインポートする
qtreeを `flexvol/qtree`形式でボリューム名として指定します。次の例では、バックエンド `ontap_nas_economy`上のFlexVol volume `flexvol1`からqtree `qtree1`をインポートします。

[listing]
----
tridentctl import volume ontap_nas_economy flexvol1/qtree1 -f <path-to-pvc-file>
----
qtree の名前を変更せずにインポートするには、 `--no-rename`フラグを追加してください。Trident がライフサイクルを管理せずに qtree をインポートするには、 `--no-manage`フラグを追加してください。

.PVCアノテーションを使用してqtreeをインポートする
以下のアノテーションを付けてPVCを作成します：
...
spec:
  ...
  storageClassName: <storage-class-name>
----
既存のFlexVol volumeとqtree名を保持するには、 `trident.netapp.io/importNoRename: "true"`アノテーションを追加します。qtreeをアンマネージドとしてインポートするには、 `trident.netapp.io/notManaged: "true"`アノテーションを追加します。

.命名と管理の動作
管理インポート（デフォルト）:: Trident は、qtree の名前を PVC 名に合わせて変更し、FlexVol ボリュームの名前を `ontap-nas-economy`命名規則に合わせて変更します。ジャンクションパスは変更されません。
`--no-rename`:: Trident は既存の FlexVol volume と qtree 名を保持します。名前はすでに Trident の命名規則に従っている必要があります。従っていない場合、Trident は qtree の削除またはマウント操作を管理できません。
`--no-manage`:: Trident は qtree を参照しますが、そのライフサイクルは管理しません。名前は Trident の命名規則に従う必要があり、Trident は qtree の自動エクスポートポリシーを使用または管理しません。


.エクスポートポリシーの動作
バックエンドが自動エクスポートポリシーを使用している場合、インポートされたqtreeはqtreeごとのエクスポートポリシーモデルを使用し、各qtreeが独自のエクスポートポリシーを持ちます。Tridentはインポート時にqtreeに空のエクスポートポリシーを割り当てます。公開時に、自動エクスポートが有効になっており、既存のFlexVolエクスポートポリシーが `default`または空の場合、TridentはFlexVolボリュームをバックエンドごとのエクスポートポリシーに更新します。

[CAUTION]
====
FlexVol ボリュームのエクスポートポリシーを更新すると、同じ FlexVol ボリューム内で現在マウントされている他の qtree に影響を与える可能性があります。qtree をインポートするのは、FlexVol ボリューム内の qtree がマウントされていない場合のみにしてください。

====
[WARNING]
====
2 つの Kubernetes クラスターを同じアクティブな SVM に向けないでください。また、別のクラスターが引き続き管理している qtree をインポートしないでください。両方のクラスターが同じボリュームを管理している場合、競合する操作によってデータが破損または消失する可能性があります。
...
----
インポート後 `lun1`、TridentはFlexVolの名前を変更します（例： `trident_lun_pool_xyz`）。同じFlexVolから追加のLUNをインポートするには、新しいFlexVol名を使用します：
...
NOTE:  `ontap-san-economy`バックエンドは一度に1つのLUNをインポートします。スクリプトを使用すれば、複数のインポートを自動化できます。
...
--
LUN の命名要件:: LUN には `lun0`という名前を付ける必要があります。
例:: もしあなたの `storagePrefix`が `xyz`の場合、LUNへの完全パスは次のとおりです：
+
--
[listing]
----
